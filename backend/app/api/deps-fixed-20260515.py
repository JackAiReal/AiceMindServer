...
from app.core.entitlement import (
    ...
    upsert_entitlement_policy_in_conn,
)


_DB_LOCK = threading.Lock()
# 首次启动必须做建表/迁移；之后在进程内再置为 True 避免重复初始化。
_DB_READY = False
...
_LEGAL_DOC_DEFAULTS: dict[str, dict[str, str]] = {
    ...
    'privacy': {
        'title': 'AiceMind 隐私政策',
        'content': '''AiceMind 隐私政策（商业回测软件适用）

更新日期：2026-05-11
生效日期：2026-05-11

一、我们收集的信息
1. 账户信息：邮箱、用户名、登录凭证（加密存储）、2FA 状态。
2. 使用信息：会话内容、策略描述与代码、回测参数、回测结果与报表元数据。
3. 设备与日志信息：IP、设备标识、访问时间、错误日志与安全审计日志。
4. 支付相关信息：订单号、支付状态、支付渠道回执（不存储银行卡完整信息）。

二、信息使用目的
1. 提供账户登录、身份验证、会员权益与额度核验服务。
2. 提供策略生成、策略校验、回测执行、报表导出等核心功能。
3. 保障系统安全，进行风控审计、异常检测、故障排查和服务优化。
4. 履行法律法规要求及处理用户投诉、争议。

三、信息共享与披露
1. 我们不会出售您的个人信息。
2. 仅在实现支付、短信/邮件通知、云基础设施托管等必要场景下，与受约束的服务提供商共享最小必要信息。
3. 在法律法规、监管要求或司法机关依法要求下，我们可能依法披露必要信息。

四、数据存储与安全
1. 采用访问控制、传输加密、日志审计、最小权限等措施保护数据安全。
2. 数据默认存储于服务运行环境；若您使用本地部署，数据由部署方自行保管与控制。
3. 我们会在实现业务目的所需期限内保存信息；超过期限后将删除或匿名化处理。

五、您的权利
您可依法行使查询、更正、删除、导出个人信息及注销账户等权利。您也可关闭账户、撤回授权（不影响撤回前基于授权的处理）。

六、未成年人保护
本产品面向具备完全民事行为能力的用户。若您为未成年人，请在监护人指导下使用。

七、跨境与第三方模型服务
当您主动启用第三方模型服务（如外部 API）时，相关请求数据可能发送至第三方服务商。请您在启用前阅读并同意其隐私条款。

八、政策更新
我们可能根据业务或法律要求更新本政策。更新后将通过系统公告、页面提示等方式通知，重大变更将以更显著方式告知。

九、联系我们
如您对本政策有疑问、意见或投诉，请通过系统内“合规与账户管理”页面联系方式与我们联系。

风险提示：本系统提供量化分析与回测工具，不构成任何投资建议。''',
    },
    'risk_disclaimer': {
...
class PlanBody(BaseModel):
    ...
    backtestEnabled: bool = True
    maxBacktestStocks: int = -1
    maxBacktestDays: int = -1
    reportDownloadEnabled: bool = True

    # 智能对话配额（-1 表示不限）
    chatMonthlyLimit: int = -1
    chatDailyLimit: int = -1
    ...
    with _db_connect() as conn:
        ...
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS client_version_policies (
                ...
                target VARCHAR(64) NOT NULL DEFAULT 'backtest-desktop',
                ...
                min_supported_version VARCHAR(64) NOT NULL DEFAULT '',
                ...
                title VARCHAR(255) NOT NULL DEFAULT '发现新版本，请升级后继续使用',
                ...
                UNIQUE(app_code, target, platform, channel)
            )
            '''
        )
        conn.execute(
            'CREATE INDEX IF NOT EXISTS idx_client_version_policies_lookup ON client_version_policies(app_code, target, platform, channel)'
        )
...
def _close_timeout_orders(
    ...
    now = _now_str()
    params: list[Any] = [now]
    where = ["o.status = 'created'", "o.expire_at != ''", 'o.expire_at <= ?']
...
def _find_recent_unpaid_trade_by_plan(
    ...
    cutoff_at = (datetime.now() - timedelta(minutes=minutes)).strftime('%Y-%m-%d %H:%M:%S')
    ...
    return conn.execute(
        ...
        WHERE t.account_id = ?
          ...
          AND t.created_at >= ?
          AND (o.expire_at = '' OR o.expire_at > ?)
        ORDER BY t.created_at DESC
        LIMIT 1
        ''',
        (account_id, plan_code, provider, cutoff_at, now_at),
    ).fetchone()
    ...
    with _DB_LOCK:
        ...
        with _db_connect() as conn:
            ...
            try:
                ...
                if 'super' in roles:
                    return
                # 兼容历史 admin 角色 & RBAC 权限
                if 'admin' in roles:
                    return
                if _has_permission(conn, user, 'system.*'):
                    return
            except Exception:
                roles = set(user.get('roles') or [])
                if {'super', 'admin'} & roles:
